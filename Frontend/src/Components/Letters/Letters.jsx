    ...
    <div style={{ background: T.bg, minHeight: '100vh', padding: '28px 32px', fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: T.textSec, margin: '5px 0 0' }}>Warranty letters, bank details, references — written on your company letterhead.</p>
        </div>
        <button onClick={() => navigate('/Letters/new')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', background: 'linear-gradient(135deg,#3b82f6,#2563eb)', color: '#fff', border: 'none', borderRadius: 11, fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(59,130,246,.35)' }}>
          <FaPlus /> New Letter
        </button>
      </div>
                ...
                <tr key={l.id} style={{ borderBottom: `1px solid ${T.border}` }}>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                      <button onClick={() => navigate(`/Letters/${l.id}/print`)} title="Preview, Print, Download & Email" style={{ padding: 7, background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 8, cursor: 'pointer', color: T.textSec }}><FaPrint size={11} /></button>
                      <button onClick={() => navigate(`/Letters/${l.id}/edit`)} title="Edit" style={{ padding: 7, background: T.surface2, border: `1px solid ${T.border}`, borderRadius: 8, cursor: 'pointer', color: T.textSec }}><FaEdit size={11} /></button>
