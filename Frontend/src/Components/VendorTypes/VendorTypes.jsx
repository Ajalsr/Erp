...
export default function VendorTypes() {
  ...
  const fetchTypes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get('/api/vendor-types/');
      setTypes(res.data?.data?.vendorTypes || []);
    } catch {
      nexusToast.error('Failed to load vendor types');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id) => {
    if (!(await confirm({ title: 'Delete vendor type?', message: 'This will permanently remove the vendor type. Vendors already using it keep their stored value.', confirmLabel: 'Delete', danger: true }))) return;
    setDeleting(id);
    try {
      await axiosInstance.delete(`/api/vendor-types/${id}`);
      nexusToast.success('Vendor type deleted');
      fetchTypes();
    } catch {
      nexusToast.error('Failed to delete vendor type');
    } finally {
      setDeleting(null);
    }
  };
    ...
    <div style={{ background: T.bg, minHeight: '100vh', padding: '28px 32px', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ display: 'flex', flexWrap: isMobile ? 'wrap' : 'nowrap', alignItems: 'center', justifyContent: 'space-between', gap: isMobile ? 10 : 0, marginBottom: 24 }}>
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: isMobile ? 19 : 22, fontWeight: 800, color: T.textPri, margin: 0, letterSpacing: '-0.03em' }}>Vendor Types</h1>
          <p style={{ fontSize: 13, color: T.textSec, margin: '4px 0 0' }}>Manage the classifications used on vendors (Individual, Manufacturer, Distributor…)</p>
        </div>
        <button onClick={() => setEditingType({ _id: null, name: '', description: '', status: 'active' })}
          ...
          <FaPlus size={11} /> New Vendor Type
        </button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 14px', height: 40, border: `1.5px solid ${T.border}`, borderRadius: 10, background: T.surface, marginBottom: 14 }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search vendor types…"
        ...
        {loading
          ...
          : filtered.length === 0
            ? <div style={{ padding: 48, textAlign: 'center' }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: T.textPri, margin: '0 0 6px' }}>No vendor types yet</p>
                <p style={{ fontSize: 12, color: T.textSec, margin: 0 }}>Create your first vendor type (e.g. Manufacturer).</p>
              </div>
            : filtered.map(t => (
...
function EditTypeDrawer({ type, T, isDark, onClose, onSaved, onDelete, deleting }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (type._id) {
        await axiosInstance.put(`/api/vendor-types/${type._id}`, form);
      } else {
        await axiosInstance.post('/api/vendor-types/', form);
      }
      nexusToast.success(type._id ? 'Vendor type updated' : 'Vendor type created');
      onSaved();
    } catch (err) {
      nexusToast.error(err?.response?.data?.message || 'Failed to save vendor type');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', justifyContent: 'flex-end' }} onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h2 style={{ fontFamily: "'Sora',sans-serif", fontSize: 16, fontWeight: 700, color: T.textPri, margin: 0 }}>{type._id ? 'Edit Vendor Type' : 'New Vendor Type'}</h2>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? 'Saving…' : type._id ? 'Update Vendor Type' : 'Create Vendor Type'}
          </button>
          {onDelete && (
