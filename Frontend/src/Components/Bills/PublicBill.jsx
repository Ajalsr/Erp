...
// so the emailed "View Bill" link always matches Print/Preview exactly.
export default function PublicBill() {
  ...
  useEffect(() => {
    api.get(`/api/bills/public/${token}`)
      .then(r => setB(r.data?.data || null))
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    api.get(`/api/bills/public/${token}/pdf`, { responseType: "blob" })
      .then(res => {
        if (cancelled) return;
  ...
  const downloadPdf = () => {
    ...
    a.href = pdfUrl;
    a.download = `bill-${b?.billNumber || token}.pdf`;
  ...
  if (notFound) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f1f5f9", fontFamily: "Inter, sans-serif" }}>
      <div style={{ textAlign: "center", color: "#64748b" }}>
        ...
        <p style={{ fontSize: 15 }}>Bill not found or this link has expired.</p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#f1f5f9", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ flexShrink: 0, background: "#1e3a5f", padding: "10px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <span style={{ color: "#fff", fontWeight: 700, fontSize: 15 }}>Spifora — Bill Portal</span>
      ...
      {loading ? (
        ...
        <iframe ref={iframeRef} src={pdfUrl} title="Bill PDF" style={{ flex: 1, width: "100%", border: "none" }} />
      )}
    </div>
  );
}
